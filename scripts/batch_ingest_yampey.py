...
# Forzar codificación UTF-8 para consola Windows
...
def main():
    if not os.path.exists(AUDIO_DIR):
        print(f"Error: Carpeta {AUDIO_DIR} no existe.", flush=True)
        return
    ...
    print(f"=== INICIO INGESTA YAMPEY ===", flush=True)
    ...
    for idx, f in enumerate(pending, 1):
        ...
        try:
            ...
            print(f"[{success_count}/{total}] OK ({duration:.1f}s) -> {title_short}", flush=True)
            update_progress_file(total, success_count, fail_count, f, title_short)

            # Pausa para respetar límites de Groq
            time.sleep(2.0)

        except Exception as e:
            print(f"[{success_count}/{total}] ERROR en {f}: {str(e)[:80]}", flush=True)
            state['failed'][f] = {'error': str(e), 'at': time.strftime("%Y-%m-%d %H:%M:%S")}
            save_state(state)
            fail_count += 1
            time.sleep(3.0)

        # Reporte periódico cada 60 segundos
        if time.time() - last_report >= 60:
            ...
            print(f"\n--- [REPORTE MINUTO A MINUTO] ---", flush=True)
    ...
    print(f"\n🎉 INGESTA COMPLETADA: {success_count} procesados con éxito, {fail_count} fallos.", flush=True)
